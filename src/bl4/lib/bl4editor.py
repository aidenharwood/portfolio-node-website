...
class ItemStats:
    ...
    item_class_raw: Optional[int] = None
@dataclass
class DecodedItem:
    serial: str
    item_type: str
    ...
    markers: Dict[str, Tuple[int, str]] = None
...
TAIL_TRIAD_LOOKUP = {
    'X': {
        'L': ('Merciless', 'Vivisecting'),
        'K': ('Pulsing Augment', 'Erupting'),
        'F': ('Concussive/Wounding', 'Wounding/Concussive Wounding')
    },
    'Y': {
        'M': 'Oscar Mike',
        'D': ['Gadget Ahoy', 'Rubberband Man'],
        'K': 'Rubberband Man',
        'G': 'Reel Big Fish',
        'J': 'GODJFC'
    },
...
BROKEN_ENDINGS = ['6PDd', '6ODd', '6GDd', '6MDd', '6mDd']
...
def bit_pack_decode(serial: str) -> Tuple[bytes, str, List[int], List[int], Dict[str, Tuple[int, str]]]:
    ...
    marker_list = ['Fme!K', '}TYg', '}TYs', 'RG}', 'RG/', '/A', '/B', '/C', '/D', '/F']
    for marker in marker_list:
        if marker in payload:
            pos = payload.index(marker)
            markers[marker] = (pos, marker)

    non_binary_positions = set()
    if 'Fme!K' in markers:
        rarity_start = markers['Fme!K'][0] + len('Fme!K')
        non_binary_positions.update(range(rarity_start, rarity_start + 5))

    for idx, c in enumerate(payload):
        if c in char_map and idx not in non_binary_positions:
            val = char_map[c]
    ...
    return bytes(byte_data), original_prefix, data_positions, char_offsets, markers
...
def decode_weapon(data: bytes, serial: str, original_prefix: str, payload: str, 
                  data_positions: List[int], char_offsets: List[int], markers: Dict[str, Tuple[int, str]]) -> DecodedItem:
    ...
    if 'Fme!K' in markers:
        ...
        rarity_start = markers['Fme!K'][0] + len('Fme!K')
        if len(payload) >= rarity_start + 5:
            fields['rarity_bundle'] = payload[rarity_start:rarity_start+5]
            if fields['rarity_bundle'] == 'V0_S6':
                stats.rarity = 5

    if 'RG}' in markers or 'RG/' in markers:
        rg_marker = 'RG}' if 'RG}' in markers else 'RG/'
        rg_start = markers[rg_marker][0] + len(rg_marker)
        effect_block = ''
        for i in range(rg_start, len(payload)):
            if payload[i] == 's':
                break
            effect_block += payload[i]
        fields['manufacturer_effects'] = effect_block

    pool_selector = None
    for pool in ['/A', '/B', '/C', '/D', '/F']:
        if pool in markers:
            pool_selector = pool
            fields['pool_selector'] = pool
            length_nibble_pos = markers[pool][0] + len(pool) + 1
            if len(payload) > length_nibble_pos and payload[length_nibble_pos-1] == '`':
                fields['length_nibble'] = payload[length_nibble_pos]

    if serial.startswith('@Ugr'):
        if len(payload) >= 3:
            fields['tail_triad'] = payload[-3:] if payload.endswith('00') else payload[-5:-2]
            stats.flags = [ord(c) for c in fields['tail_triad']]
    ...
    if 'byte_8' in fields:
        ...
        stats.item_class_raw = fields['byte_8']
    if 'byte_1' in fields:
        stats.rarity = fields['byte_1'] if 'rarity_bundle' not in fields else stats.rarity
    ...
    confidence = "high" if len(data) in [24, 26] and 'Fme!K' in markers else "medium"
...
def decode_equipment_e(data: bytes, serial: str, original_prefix: str, payload: str, data_positions: List[int], char_offsets: List[int], markers: Dict[str, Tuple[int, str]]) -> DecodedItem:
    fields = extract_fields(data)
    stats = ItemStats()
    ...
    if 'byte_3' in fields:
        ...
        stats.item_class_raw = fields['byte_3']
    if 'byte_9' in fields:
...
def decode_equipment_d(data: bytes, serial: str, original_prefix: str, payload: str, data_positions: List[int], char_offsets: List[int], markers: Dict[str, Tuple[int, str]]) -> DecodedItem:
    fields = extract_fields(data)
    stats = ItemStats()
    ...
    if 'byte_6' in fields:
        ...
        stats.item_class_raw = fields['byte_6']
...
def decode_other_type(data: bytes, serial: str, item_type: str, original_prefix: str, payload: str, data_positions: List[int], char_offsets: List[int], markers: Dict[str, Tuple[int, str]]) -> DecodedItem:
    fields = extract_fields(data)
    stats = ItemStats()
...
def decode_item_serial(serial: str) -> DecodedItem:
    try:
        data, original_prefix, data_positions, char_offsets, markers = bit_pack_decode(serial)
        ...
        if item_type == 'r':
            return decode_weapon(data, serial, original_prefix, payload, data_positions, char_offsets, markers)
        elif item_type == 'e':
            return decode_equipment_e(data, serial, original_prefix, payload, data_positions, char_offsets, markers)
        elif item_type == 'd':
            return decode_equipment_d(data, serial, original_prefix, payload, data_positions, char_offsets, markers)
        else:
            return decode_other_type(data, serial, item_type, original_prefix, payload, data_positions, char_offsets, markers)
...
def encode_item_serial(decoded_item: DecodedItem) -> str:
    try:
        ...
        markers = decoded_item.markers or {}
        ...
        if decoded_item.item_type == 'r':
            ...
            if stats.level is not None and stats.level != decoded_item.raw_fields.get('byte_13', None) and len(data) >= 14:
                data[13] = stats.level
                modified = True
            if decoded_item.serial.startswith('@Ugr') and 'tail_triad' in decoded_item.raw_fields:
                tail_triad = decoded_item.raw_fields['tail_triad']
                if stats.flags and len(stats.flags) == 3:
                    new_triad = ''.join(chr(c) for c in stats.flags)
                    broken_endings = ['6PDd', '6ODd', '6GDd', '6MDd', '6mDd']
                    if new_triad + '00'[:1] in broken_endings:
                        raise ValueError(f"Invalid tail triad: {new_triad}")
                    decoded_item.original_payload = decoded_item.original_payload[:-5] + new_triad + '00'
        ...
        if 'pool_selector' in decoded_item.raw_fields and 'length_nibble' in decoded_item.raw_fields:
            pool = decoded_item.raw_fields['pool_selector']
            nibble = decoded_item.raw_fields['length_nibble']
            if pool == '/F' and nibble != '5':
                raise ValueError("Pool /F requires length nibble 5")
            pool_pos = markers[pool][0]
            decoded_item.original_payload = (
                decoded_item.original_payload[:pool_pos] +
                pool + '`' + nibble +
                decoded_item.original_payload[pool_pos+len(pool)+2:]
            )
...
class GearNGunEditor:
    ...
    def display_stats(self):
        ...
        fields = self.decoded_item.raw_fields
        ...
        if 'family_marker' in fields:
            labels.append(("Family Marker:", "Item family identifier (e.g., Fme!K)", "family_marker"))
        if 'rarity_bundle' in fields:
            labels.append(("Rarity Bundle:", "Rarity code (e.g., U~Euo)", "rarity_bundle"))
        if 'manufacturer_effects' in fields:
            labels.append(("Manufacturer Effects:", "Manufacturer-specific effects", "manufacturer_effects"))

        if self.decoded_item.serial.startswith('@Ugr'):
            labels.append(("Tail Triad:", "Perk family, firmware, tier (X,Y,Z)", "flags"))
            labels.append(("Pool Selector:", "Mod pool (/A, /B, /C, /D, /F)", "pool_selector"))
            labels.append(("Length Nibble:", "Roll count validator (e.g., 5 for /F)", "length_nibble"))
            labels.append(("Archetype:", "Knife or Grenade (e.g., t=Knife, b=Grenade)", "archetype"))
        ...
        for label_text, desc, attr in labels:
            ...
            if attr == "flags" and stats.flags:
                entry.insert(0, ''.join(chr(c) for c in stats.flags))
            elif attr in ["family_marker", "rarity_bundle", "manufacturer_effects", "pool_selector", "length_nibble", "archetype"]:
                entry.insert(0, str(fields.get(attr, "")))
            else:
    ...
    def save_to_text(self):
        ...
        stat_lines = [
            ...
            f"Tail Triad: {''.join(chr(c) for c in stats.flags) if stats.flags else 'N/A'}"
        ]
        text_content += "\n".join(stat_lines) + "\n\n"

        text_content += f"STRUCTURAL FIELDS:\n"
        text_content += f"Family Marker: {self.decoded_item.raw_fields.get('family_marker', 'N/A')}\n"
        text_content += f"Rarity Bundle: {self.decoded_item.raw_fields.get('rarity_bundle', 'N/A')}\n"
        text_content += f"Manufacturer Effects: {self.decoded_item.raw_fields.get('manufacturer_effects', 'N/A')}\n"
        text_content += f"Pool Selector: {self.decoded_item.raw_fields.get('pool_selector', 'N/A')}\n"
        text_content += f"Length Nibble: {self.decoded_item.raw_fields.get('length_nibble', 'N/A')}\n"
        text_content += f"Archetype: {self.decoded_item.raw_fields.get('archetype', 'N/A')}\n\n"
        ...
        for key, value in self.decoded_item.raw_fields.items():
            if key not in ['potential_stats', 'potential_flags', 'family_marker', 'rarity_bundle', 
                           'manufacturer_effects', 'pool_selector', 'length_nibble', 'archetype']:
    ...
    def open_raw_data(self):
        ...
        for key, value in self.decoded_item.raw_fields.items():
            if key not in ['family_marker', 'rarity_bundle', 'manufacturer_effects', 'pool_selector', 'length_nibble', 'archetype']:
                frame = tk.Frame(inner_frame, bg="#2C2F33")
                frame.pack(fill="x", pady=2)
    ...
    def save_raw_changes(self):
        if not self.decoded_item:
            messagebox.showerror("Error", "No decoded item to save changes for.")
            return

        original_serial = self.decoded_item.serial
        original_binary = self.decoded_item.original_binary
        original_payload = self.decoded_item.original_payload
        ...
        for key in self.decoded_item.raw_fields:
            if hasattr(self, f"raw_{key}_entry"):
                ...
                try:
                    old_value = self.decoded_item.raw_fields[key]
                    if 'val16_at' in key or 'header' in key or 'field' in key:
                        new_value = int(value_str)
                        if new_value != old_value:
                            self.decoded_item.raw_fields[key] = new_value
                            modified_fields.append((key, new_value))
                    elif 'byte_' in key:
                        new_value = int(value_str)
                        if new_value != old_value:
                            self.decoded_item.raw_fields[key] = new_value
                            modified_fields.append((key, new_value))
                    elif key == 'potential_stats':
                        ...
                        if new_value != old_value:
                            self.decoded_item.raw_fields[key] = new_value
                            modified_fields.append((key, new_value))
                    elif key in ['family_marker', 'rarity_bundle', 'manufacturer_effects', 'pool_selector', 'length_nibble', 'tail_triad', 'archetype']:
                        if value_str != old_value:
                            self.decoded_item.raw_fields[key] = value_str
                            modified_fields.append((key, value_str))
                except Exception:
                    continue

        for key in ['family_marker', 'rarity_bundle', 'manufacturer_effects']:
            if hasattr(self, f"{key}_entry"):
                entry = getattr(self, f"{key}_entry")
                value_str = entry.get().strip()
                try:
                    old_value = self.decoded_item.raw_fields.get(key, "")
                    if value_str != old_value:
                        self.decoded_item.raw_fields[key] = value_str
                        modified_fields.append((key, value_str))
                except Exception:
        ...
        if self.decoded_item.item_type == 'r':
            if 'val16_at_0' in self.decoded_item.raw_fields:
                self.decoded_item.stats.primary_stat = self.decoded_item.raw_fields['val16_at_0']
            if 'val16_at_12' in self.decoded_item.raw_fields:
                self.decoded_item.stats.secondary_stat = self.decoded_item.raw_fields['val16_at_12']
            if 'byte_1' in self.decoded_item.raw_fields:
                self.decoded_item.stats.rarity = self.decoded_item.raw_fields['byte_1']
            if 'byte_4' in self.decoded_item.raw_fields:
                self.decoded_item.stats.manufacturer = self.decoded_item.raw_fields['byte_4']
            if 'byte_8' in self.decoded_item.raw_fields:
                self.decoded_item.stats.item_class = self.decoded_item.raw_fields['byte_8']
            if 'byte_13' in self.decoded_item.raw_fields:
                self.decoded_item.stats.level = self.decoded_item.raw_fields['byte_13']
        elif self.decoded_item.item_type == 'e':
            if 'val16_at_2' in self.decoded_item.raw_fields:
                self.decoded_item.stats.primary_stat = self.decoded_item.raw_fields['val16_at_2']
            if 'val16_at_8' in self.decoded_item.raw_fields:
                self.decoded_item.stats.secondary_stat = self.decoded_item.raw_fields['val16_at_8']
            if 'byte_1' in self.decoded_item.raw_fields:
                self.decoded_item.stats.manufacturer = self.decoded_item.raw_fields['byte_1']
            if 'byte_3' in self.decoded_item.raw_fields:
                self.decoded_item.stats.item_class = self.decoded_item.raw_fields['byte_3']
            if 'byte_9' in self.decoded_item.raw_fields:
                self.decoded_item.stats.rarity = self.decoded_item.raw_fields['byte_9']
            if 'val16_at_10' in self.decoded_item.raw_fields:
                self.decoded_item.stats.level = self.decoded_item.raw_fields['val16_at_10']
        elif self.decoded_item.item_type == 'd':
            if 'val16_at_4' in self.decoded_item.raw_fields:
                self.decoded_item.stats.primary_stat = self.decoded_item.raw_fields['val16_at_4']
            if 'val16_at_8' in self.decoded_item.raw_fields:
                self.decoded_item.stats.secondary_stat = self.decoded_item.raw_fields['val16_at_8']
            if 'byte_5' in self.decoded_item.raw_fields:
                self.decoded_item.stats.manufacturer = self.decoded_item.raw_fields['byte_5']
            if 'byte_6' in self.decoded_item.raw_fields:
                self.decoded_item.stats.item_class = self.decoded_item.raw_fields['byte_6']
            if 'byte_14' in self.decoded_item.raw_fields:
                self.decoded_item.stats.rarity = self.decoded_item.raw_fields['byte_14']
            if 'val16_at_10' in self.decoded_item.raw_fields:
                self.decoded_item.stats.level = self.decoded_item.raw_fields['val16_at_10']
        else:
            potential_stats = self.decoded_item.raw_fields.get('potential_stats', [])
            if potential_stats:
                self.decoded_item.stats.primary_stat = potential_stats[0][1] if len(potential_stats) > 0 else None
                self.decoded_item.stats.secondary_stat = potential_stats[1][1] if len(potential_stats) > 1 else None
            if 'byte_1' in self.decoded_item.raw_fields:
                self.decoded_item.stats.manufacturer = self.decoded_item.raw_fields['byte_1']
            if 'byte_2' in self.decoded_item.raw_fields:
                self.decoded_item.stats.rarity = self.decoded_item.raw_fields['byte_2']
            if 'byte_3' in self.decoded_item.raw_fields:
                self.decoded_item.stats.item_class = self.decoded_item.raw_fields['byte_3']
            if 'val16_at_10' in self.decoded_item.raw_fields:
                self.decoded_item.stats.level = self.decoded_item.raw_fields['val16_at_10']

        if 'tail_triad' in self.decoded_item.raw_fields and self.decoded_item.serial.startswith('@Ugr'):
            new_triad = self.decoded_item.raw_fields['tail_triad']
            if len(new_triad) == 3:
                self.decoded_item.stats.flags = [ord(c) for c in new_triad]

        if modified_fields:
            if 'rarity_bundle' in self.decoded_item.raw_fields and any(k == 'rarity_bundle' for k, _ in modified_fields):
                if 'Fme!K' in self.decoded_item.markers:
                    rarity_start = self.decoded_item.markers['Fme!K'][0] + len('Fme!K')
                    new_rarity_bundle = self.decoded_item.raw_fields['rarity_bundle']
                    if len(new_rarity_bundle) == 5:
                        self.decoded_item.original_payload = (
                            self.decoded_item.original_payload[:rarity_start] +
                            new_rarity_bundle +
                            self.decoded_item.original_payload[rarity_start+5:]
                        )
                        modified = True

            if 'tail_triad' in self.decoded_item.raw_fields and any(k == 'tail_triad' for k, _ in modified_fields) and self.decoded_item.serial.startswith('@Ugr'):
                new_triad = self.decoded_item.raw_fields['tail_triad']
                if len(new_triad) == 3:
                    if new_triad + '00'[:1] in BROKEN_ENDINGS:
                        messagebox.showwarning("Warning", f"Tail triad {new_triad} may brick the item!")
                    self.decoded_item.original_payload = self.decoded_item.original_payload[:-5] + new_triad + '00'
                    modified = True

            if 'pool_selector' in self.decoded_item.raw_fields and 'length_nibble' in self.decoded_item.raw_fields:
                pool = self.decoded_item.raw_fields['pool_selector']
                nibble = self.decoded_item.raw_fields['length_nibble']
                if pool in ['/A', '/B', '/C', '/D', '/F'] and nibble in ['4', '5', '6']:
                    if pool == '/F' and nibble != '5':
                        messagebox.showwarning("Warning", "Pool /F requires length nibble 5")
                    if pool in self.decoded_item.markers:
                        pool_pos = self.decoded_item.markers[pool][0]
                        self.decoded_item.original_payload = (
                            self.decoded_item.original_payload[:pool_pos] +
                            pool + '`' + nibble +
                            self.decoded_item.original_payload[pool_pos+len(pool)+2:]
                        )
                        modified = True

            if 'archetype' in self.decoded_item.raw_fields and 'itbd' in self.decoded_item.original_payload:
                archetype = self.decoded_item.raw_fields['archetype']
                if archetype in ['t', 'b']:
                    pos = self.decoded_item.original_payload.index('itbd') + 2
                    self.decoded_item.original_payload = (
                        self.decoded_item.original_payload[:pos] +
                        archetype +
                        self.decoded_item.original_payload[pos+1:]
                    )
                    modified = True

        modified_binary = False
        if modified_fields:
            has_val16_or_byte = any('val16_at' in k or 'byte_' in k for k, _ in modified_fields)
            has_potential_stats_or_flags = any(k in ['potential_stats', 'potential_flags'] for k, _ in modified_fields)

            if not has_val16_or_byte and not has_potential_stats_or_flags:
                for key, value in modified_fields:
                    if key in ['header_le', 'header_be', 'field2_le', 'field3_le']:
                        try:
                            if key == 'header_le' and len(data) >= 4:
                                struct.pack_into('<I', data, 0, value)
                                modified_binary = True
                            elif key == 'header_be' and len(data) >= 4:
                                struct.pack_into('>I', data, 0, value)
                                modified_binary = True
                            elif key == 'field2_le' and len(data) >= 8:
                                struct.pack_into('<I', data, 4, value)
                                modified_binary = True
                            elif key == 'field3_le' and len(data) >= 12:
            ...
            for key, value in self.decoded_item.raw_fields.items():
                if 'val16_at' in key and any(k == key for k, _ in modified_fields):
                    try:
            ...
            for key, value in self.decoded_item.raw_fields.items():
                if 'byte_' in key and any(k == key for k, _ in modified_fields):
                    try:
                        pos = int(key.split('_')[-1])
                        if len(data) > pos:
                            data[pos] = value
            ...
            if 'potential_stats' in self.decoded_item.raw_fields and any(k == 'potential_stats' for k, _ in modified_fields):
                for pos, val in self.decoded_item.raw_fields['potential_stats']:
                    if len(data) > pos + 1:
                        struct.pack_into('<H', data, pos, val)
                        modified_binary = True

            if 'potential_flags' in self.decoded_item.raw_fields and any(k == 'potential_flags' for k, _ in modified_fields):
                for pos, val in self.decoded_item.raw_fields['potential_flags']:
                    if len(data) > pos:
                        data[pos] = val
                        modified_binary = True

        if modified_binary:
            new_binary = bytes(data)
            if new_binary != original_binary:
                self.decoded_item.original_binary = new_binary
                modified = True

        if modified or modified_fields:
            try:
                self.new_serial = bit_pack_encode(
                    self.decoded_item.original_binary,
                    self.decoded_item.original_prefix,
                    self.decoded_item.original_payload,
                    self.decoded_item.data_positions,
                    self.decoded_item.char_offsets
                )
                if self.new_serial != original_serial:
                    self.decoded_item.serial = self.new_serial
                    self.output_label.config(text=f"New Serial: {self.new_serial}")
                    self.copy_button.config(state="normal")
                    messagebox.showinfo("Success", f"Raw changes saved and serial updated to:\n{self.new_serial}")
                else:
                    messagebox.showinfo("Info", "Raw changes saved, but serial remains unchanged.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to encode new serial: {str(e)}")
        else:
            messagebox.showinfo("Info", "No valid changes detected.")
    ...
    def save_and_encode(self):
        ...
        try:
            ...
            if hasattr(self, "flags_entry"):
                value = self.flags_entry.get().strip()
                if value and len(value) == 3:
                    stats.flags = [ord(c) for c in value]
                    broken_endings = ['6PDd', '6ODd', '6GDd', '6MDd', '6mDd']
                    if value + '00'[:1] in broken_endings:
                        messagebox.showwarning("Warning", f"Tail triad {value} may brick the item!")
            if hasattr(self, "family_marker_entry"):
                value = self.family_marker_entry.get().strip()
                if value:
                    fields['family_marker'] = value
            if hasattr(self, "rarity_bundle_entry"):
                value = self.rarity_bundle_entry.get().strip()
                if value:
                    fields['rarity_bundle'] = value
            if hasattr(self, "manufacturer_effects_entry"):
                value = self.manufacturer_effects_entry.get().strip()
                if value:
                    fields['manufacturer_effects'] = value
            if hasattr(self, "pool_selector_entry"):
                value = self.pool_selector_entry.get().strip()
                if value in ['/A', '/B', '/C', '/D', '/F']:
                    fields['pool_selector'] = value
                    if value == '/F' and fields.get('length_nibble') != '5':
                        messagebox.showwarning("Warning", "Pool /F requires length nibble 5")
            if hasattr(self, "length_nibble_entry"):
                value = self.length_nibble_entry.get().strip()
                if value in ['4', '5', '6']:
                    fields['length_nibble'] = value
            if hasattr(self, "archetype_entry"):
                value = self.archetype_entry.get().strip()
                if value in ['t', 'b']:
                    fields['archetype'] = value
                    if 'itbd' in self.decoded_item.original_payload:
                        pos = self.decoded_item.original_payload.index('itbd') + 2
                        self.decoded_item.original_payload = (
                            self.decoded_item.original_payload[:pos] +
                            value +
            ...
            if self.decoded_item.weapon_name:     
                self.output_label.config(text=f"New Serial for {self.decoded_item.weapon_name}: {self.new_serial}")
                self.copy_button.config(state="normal")
